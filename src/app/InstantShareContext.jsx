...
export default function InstantShareProvider({ children }) {
  ...
  const joinWithCode = useCallback((code6) => {
  ...
  const processFiles = useCallback((selectedFiles) => {
    if (!isConnected) { showToast("Please join a session first", "error"); return; }
    selectedFiles.forEach((file) => {
      const fileObj = { id: Math.random().toString(36).slice(2), name: file.name, size: file.size, status: "uploading", progress: 0 };
      ...
      const timer = setInterval(() => {
        cur += step;
        setFiles((prev) => prev.map((f) => f.id === fileObj.id ? { ...f, progress: Math.min(cur, 100) } : f));
        if (cur >= 100) {
          clearInterval(timer);
          setFiles((prev) => prev.map((f) => f.id === fileObj.id ? { ...f, status: "uploaded", progress: 100 } : f));
          busRef.current.emit("fileUploaded", { ...fileObj, status: "uploaded", progress: 100 });
  ...
  const downloadFile = useCallback((id) => {
    setFiles((prev) => prev.map((f) => f.id === id ? { ...f, status: "downloaded" } : f));
  ...
  const toggleAutoDownload = useCallback(() => {
    setAutoDownload((v) => {
      const nv = !v; showToast(nv ? "Auto-download enabled" : "Auto-download disabled"); return nv;
    });
  }, [showToast]);
  ...
  // Auto-download behavior on receive when upload finishes
  useEffect(() => {
  ...
  const value = useMemo(() => ({
    // state
    currentSession, autoDownload, timeLeft, files, isConnected,
    // utils
    formatTime, formatFileSize,
    // actions
    createPairing, joinWithCode, processFiles, downloadFile, toggleAutoDownload, resetSession,
    // bus
    bus: busRef.current,
    showToast,
